...
function countVowels(str) {
  const vowelsRegex = /[aeiouyAEIOUY]/g;
...
function invertCase(str) {
  const invertedString = Array.from(str, (char) => {
...
function extractNameFromTemplate(value) {
  ...
  if (helloIndex !== -1 && exclamationIndex !== -1) {
    const name = value.substring(helloIndex + 7, exclamationIndex);
    return name.trim();
  }
  return '';
}
...
function getCardId(value) {
  // Массив с порядковыми номерами карт в стандартной колоде
  const cards = [
    'A♣',
    '2♣',
    '3♣',
  ...
  // Используем метод indexOf для поиска индекса карты в массиве
